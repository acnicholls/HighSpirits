...
Begin VB.Form frmShipping 
   Caption         =   "Hight Spirits Shipping"
   ClientHeight    =   3555
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   8385
...
Private Sub cmdUpdate_Click()
    ...
    If Val(txtShipped.Text) > Val(lblActualData.Caption) Then
        gstrMessage = "The current transaction can't be completed (amount shipped higher then inventory."
        gstrTitle = "Invalid transaction  ..."
        gintStyle = vbOKOnly + vbExclamation
        gintAnswer = MsgBox(gstrMessage, gintStyle, gstrTitle)
        txtShipped.SetFocus
        Exit Sub
    ...
    Do While Not EOF(intFileHandleInput)
        ...
        End If
    Loop
    msngQuantity = Val(lblActualData.Caption) - Val(txtShipped.Text)
    ...
    Write #mintFileHandle, strCurrentTransactionID; "S"; Format(Date, "mm/dd/yyyy"); strUPCToUpdate _
                                          ; Val(lblActualData.Caption); Val(txtShipped.Text); msngQuantity; msngPrice
    Close
...
Private Sub txtShipped_GotFocus()

    txtShipped.SelStart = 0
    txtShipped.SelLength = Len(txtShipped.Text)

End Sub

Private Sub txtShipped_KeyPress(KeyAscii As Integer)
